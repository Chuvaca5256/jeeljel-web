...
const { fetchStandings, fetchTopScorers } = require('../services/standingsService')

const router = express.Router()

router.get('/:ligaId/scorers', async (req, res) => {
  try {
    ...
    const data = await fetchTopScorers(ligaId, redis)

    if (!data) {
      return res.status(502).json({ error: 'No se pudieron obtener goleadores' })
    }

    res.json(data)
  } catch (err) {
    console.error('[ollin][route] /standings/:ligaId/scorers:', err.message)
    res.status(500).json({ error: 'Error leyendo goleadores' })
  }
})
